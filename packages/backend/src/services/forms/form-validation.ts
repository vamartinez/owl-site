/**
 * Form response validation module.
 *
 * Pure validation logic for submitted form field values.
 * This module is designed to be usable both server-side (Lambda) and
 * extractable for client-side validation (React).
 *
 * NO dependencies on DynamoDB, AWS SDK, or any backend-specific imports.
 *
 * Requirements: 10.1, 10.2, 10.3, 10.4, 10.5, 10.7
 */

import { FieldType, ALLOWED_FILE_TYPES, MAX_FILE_SIZE_MB } from './types.js';
import type { FieldConfig } from './types.js';

// ─── Validation Error Interface ───────────────────────────────────────────────

export interface FieldValidationError {
  /** The field_id of the field that failed validation */
  field_id: string;
  /** The label of the field (for display purposes) */
  field_label: string;
...
export interface FileMetadata {
  ...
  /** File size in bytes */
  size_bytes: number;
  /** MIME type or file extension (e.g., 'application/pdf', 'image/jpeg', 'pdf', 'jpeg') */
  content_type: string;
}
 ...
 *
 * @param fields - The form field configurations (schema)
 * @param answers - The submitted answers keyed by field_id
 * @param fileMetadata - Optional file metadata keyed by field_id (for carga_archivo fields)
...
function isEmpty(value: unknown, fieldType: FieldType): boolean {
  ...
  switch (fieldType) {
    ...
    case FieldType.SELECCION_MULTIPLE:
      // Only consider truly empty values (null, undefined, empty array) as "empty"
      // A non-array value like a string is NOT empty — it's an invalid type that should be validated
      if (value === null || value === undefined) return true;
      if (Array.isArray(value)) return value.length === 0;
      return false; // Non-array, non-null values should proceed to type validation
...
/**
 * Validates text fields (texto_corto, texto_largo).
 * Checks: min_length, max_length, pattern (regex).
 */
...
/**
 * Validates numeric fields (numero).
 * Checks: numeric format, min_value, max_value.
 */
function validateNumeric(field: FieldConfig, value: unknown): FieldValidationError[] {
  ...
  // Range check: report both min and max in the message when out of range
...
/**
 * Validates date fields (fecha).
 * Checks: format (ISO 8601 YYYY-MM-DD), valid date, min_date, max_date.
 */
function validateDate(field: FieldConfig, value: unknown): FieldValidationError[] {
  ...
  // Valid date check (e.g., 2024-02-30 is invalid)
...
/**
 * Validates single selection fields (seleccion_simple).
 * Checks: value must be one of the configured option IDs.
 */
...
/**
 * Validates multiple selection fields (seleccion_multiple).
 * Checks: all values must be from configured option IDs.
 */
...
/**
 * Validates checkbox acceptance fields (checkbox_aceptacion).
 * If required, value must be true.
 */
function validateCheckbox(field: FieldConfig, value: unknown): FieldValidationError[] {
  ...
  // If required and value is false, it's invalid
...
/**
 * Normalizes a content type or extension to a simple extension string.
 */
...
/**
 * Validates file upload fields (carga_archivo).
 * Checks: file type (PDF, JPEG, PNG), file size (max 10 MB).
 */
function validateFile(field: FieldConfig, fileMeta?: FileMetadata): FieldValidationError[] {
  const errors: FieldValidationError[] = [];

  if (!fileMeta) {
    // If required, this would have been caught by the required check above
    return errors;
  }

  // Determine allowed types from field config or use defaults
