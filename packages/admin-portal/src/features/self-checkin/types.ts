/**
 * Self Check-In frontend domain types.
 *
 * Mirrors the backend self-checkin service contracts:
 *  - Admin (authenticated): site token generate/regenerate, send SMS link.
 *  - Public (no auth): token resolution + identity verify + worker-scoped decision.
 *
 * The public surface deliberately exposes ONLY the WorkerView decision fields
...
/** Response of POST /checkin/sites/{siteId}/token[/regenerate]. */
...
/** Response of POST /checkin/sms-link. */
export interface SendSmsLinkResponse {
  sent: boolean;
  expires_at?: string;
}
