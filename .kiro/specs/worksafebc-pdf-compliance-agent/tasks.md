...
- [ ] 1. Data model: new DynamoDB tables and CDK wiring
  - [x] 1.1 Add `AnalysisSessions`, `RegulatoryClauses`, `RegulatoryVersions` table definitions to the data stack
...
- [ ] 3. Regulatory Knowledge Base: versioned clause management
  - [x] 3.1 Implement `regulatory-kb-manager.ts`
  ...
  - [x] 3.4 Add API routes: `POST /worksafebc-agent/regulatory-versions`, `GET /worksafebc-agent/regulatory-versions`
...
- [ ] 4. Upload and categorization endpoints
  - [x] 4.1 Implement `POST /worksafebc-agent/sessions`
  ...
  - [x]* 4.2 Write property test: upload acceptance is format/size bounded
...
- [ ] 6. Async pipeline infrastructure
  - [x] 6.1 Add `pdf-compliance-analysis-queue` SQS queue and SNS state-change topic to the events stack
  ...
  - [x] 6.2 Add two new Lambda entry points sharing the `report-validation` deployment bundle
...
- [ ] 7. Extraction consumer (reuses text-extractor.ts)
  - [x] 7.1 Implement `extractionConsumerHandler`
  ...
  - [x]* 7.2 Write property test: extraction never reaches an undefined terminal state
...
- [ ] 8. WorkSafeBC validation engine (new module)
  - [x] 8.1 Implement `worksafebc-validation-engine.ts`: `selectApplicableParts`
  ...
  - [x] 8.2 Implement `classifySeverity` and `assignComplianceLevel` as pure functions
  ...
  - [x]* 8.3 Write property test: severity classification totality
    - **Property 4: Severity classification is total and deterministic**
    - **Validates: Requirements 3.5**

  - [x]* 8.4 Write property test: compliance-level assignment is a pure function of findings
  ...
  - [x] 8.5 Implement `analyzeCompliance(input): Promise<AnalyzeResult>`
  ...
  - [x] 8.6 Implement `analysisConsumerHandler`
...
- [ ] 10. Session history, retrieval, and tenant isolation
  - [x] 10.1 Implement `GET /worksafebc-agent/sessions/{id}`
  ...
  - [x]* 10.2 Write property test: tenant isolation on session access
  ...
  - [x] 10.3 Implement `GET /worksafebc-agent/sessions` (history, paginated)
  ...
  - [x] 10.4 Implement `POST /worksafebc-agent/sessions/{id}/reanalyze`
...
- [ ] 12. JSON schema, parser, and pretty-printer (independent — can be built in parallel with tasks 3-11)
  - [x] 12.1 Implement `report-schema.ts` as a pure module (no AWS SDK imports)
  ...
  - [x]* 12.2 Write property test: pretty-print round-trip is byte-identical
  ...
  - [x]* 12.3 Write property test: parser rejects malformed input with precise diagnostics
  ...
  - [x] 12.4 Implement `GET /worksafebc-agent/sessions/{id}/report/export?format=json`
...
- [ ] 14. Frontend: Admin Portal components
  - [x] 14.1 Create `UploadDocumentModal` component
  ...
  - [x] 14.2 Create `AnalysisProgress` component
  ...
  - [x] 14.3 Create `AnalysisSessionList` page
  ...
  - [x] 14.4 Create `ComplianceReportView` component
  ...
  - [x] 14.5 Create `RegulatoryKBAdmin` page (platform_admin only)
    - Form to publish a new regulatory version (effective date + clauses), list of published versions with change summaries
    - _Requirements: 7.3, 7.6_

  - [x]* 14.6 Write component tests for `AnalysisProgress` and `ComplianceReportView`
